...
- [ ] T001 Confirm project structure matches plan.md (backend/src/, frontend/src/)
...
- [ ] T002 [P] Add `Stroke`, `Point`, and `GuessEntry` types to `backend/src/models/game.ts`
- [ ] T003 [P] Update `Room` interface in `backend/src/models/game.ts` to add `scores`, `guessHistory`, `canvasStrokes`
- [ ] T004 Update `RoomSnapshot` interface in `backend/src/models/game.ts` to add `scores`, `guessHistory`, `canvasStrokes`
- [ ] T005 [P] Update `Participant` interface in `backend/src/models/game.ts` to add `score` (derived from `room.scores`)
- [ ] T006 Update `toRoomSnapshot` in `backend/src/services/roomStore.ts` to include new fields and map participant scores
- [ ] T007 Update `startGame` in `backend/src/services/roomStore.ts` to initialize `scores` to 0, `guessHistory` and `canvasStrokes` to empty arrays
- [ ] T008 Update `removeParticipant` in `backend/src/services/roomStore.ts` to clear round state (`scores`, `guessHistory`, `canvasStrokes`) when drawer leaves during `playing`
- [ ] T009 [P] Update frontend `RoomSnapshot` and `Participant` types in `frontend/src/services/api.ts`
- [ ] T010 [P] Update `roomStore.ts` frontend state types to include new room snapshot fields
- [ ] T011 [P] Update existing backend tests in `backend/src/services/roomStore.test.ts` to account for new model fields
- [ ] T012 [P] Update existing frontend API tests in `frontend/src/services/api.test.ts` to account for new snapshot fields
...
- [ ] T013 [P] [US1] Add Zod schemas for canvas stroke (`updateCanvasSchema`) and clear (`clearCanvasSchema`) in `backend/src/api/schemas.ts`
- [ ] T014 [US1] Implement `addStroke` and `clearCanvas` service methods in `backend/src/services/roomStore.ts`
- [ ] T015 [US1] Add `POST /rooms/:code/canvas` and `POST /rooms/:code/canvas/clear` routes in `backend/src/api/rooms.ts`
- [ ] T016 [P] [US1] Add `submitStroke` and `clearCanvas` API methods in `frontend/src/services/api.ts`
- [ ] T017 [US1] Create `Canvas.tsx` component in `frontend/src/components/Canvas.tsx` with freehand drawing, normalized 0–1000 coordinates, and clear button
- [ ] T018 [US1] Integrate `Canvas` into `frontend/src/pages/GamePage.tsx` replacing the canvas placeholder
...
- [ ] T019 [P] [US2] Add Zod `submitGuessSchema` in `backend/src/api/schemas.ts`
- [ ] T020 [US2] Implement `submitGuess` in `backend/src/services/roomStore.ts` with trim, empty rejection, case-insensitive compare, 100 points for first correct guess per player, 0 otherwise
- [ ] T021 [US2] Add `POST /rooms/:code/guess` route in `backend/src/api/rooms.ts`
- [ ] T022 [P] [US2] Add `submitGuess` API method in `frontend/src/services/api.ts`
- [ ] T023 [US2] Update `GuessForm.tsx` to call `submitGuess`, show errors, and disable for drawer
...
- [ ] T024 [US3] Update `ResultPanel.tsx` to render `guessHistory` from the room snapshot with guess text, submitter name, and correct/incorrect indicator
...
- [ ] T025 [US4] Update `Scoreboard.tsx` to display participants and their scores from the room snapshot `scores` field
...
- [ ] T026 [P] Run backend test suite: `cd backend && npm test`
- [ ] T027 [P] Run frontend test suite: `cd frontend && npm test`
- [ ] T028 Run `npm run build` in both `/backend` and `/frontend`
- [ ] T029 Validate all quickstart.md scenarios manually (draw, clear, correct guess, case variant, empty guess, incorrect guess, drawer cannot guess, history sync, scoreboard, drawer leave mid-round)
